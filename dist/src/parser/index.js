...
class Parser {
    ...
    static applyMutations(memo, mutations) {
        ...
        const music_multi_select_menu_items = memo.getType(MusicMultiSelectMenuItem_1.default);
        if (music_multi_select_menu_items.length > 0 && !mutations) {
            ...
            if (missing_or_invalid_mutations.length > 0) {
                console.warn(new Utils_1.InnertubeError(`Mutation data missing or invalid for ${missing_or_invalid_mutations.length} out of ${music_multi_select_menu_items.length} MusicMultiSelectMenuItems. ` +
